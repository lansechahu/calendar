...
function update() {
	...
	//tempObj.gzYear是天干地支，tempObj.Animal是生肖
	var tempObj = calendar.solar2lunar(y, m + 1, aMouth[m]);
	lunarYear = tempObj.gzYear + '(' + tempObj.Animal + ')年';
	...
	function setInfo(__y, __m, __d, __dayNum) {
		...
		//获取阴历日期及节气
		var obj = calendar.solar2lunar(temp.year, temp.month, temp.date);
		temp.cYear = obj.gzYear + '(' + obj.Animal + ')年';
		temp.cMonth = obj.IMonthCn;
		temp.cDay = obj.IDayCn;
		temp.solarTerm = obj.Term || '';
...
function showFullDate() {
	...
	for(var i = 0; i < total; i++) {
		...
		if(tempDay.canClick) temp.on('click', function() {
			selectDay($(this));
		});
	}
}
...
function selectDay(__temp) {
	...
	__this.addClass('boxLineSelect');
	var str = __this.attr('year') + '年' + __this.attr('month') + '月' + __this.attr('date') + '日 ' + __this.attr('day');
	$('#selectYM').html(str);

	//显示阴历部分
	var lunarStr = __this.attr('cYear') + __this.attr('cMonth') + __this.attr('cDay');

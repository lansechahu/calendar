...
function GetcDateString(solarYear, solarMonth, solarDay) {
    var tmp = "";
    //tmp += GetcYearString();
    var lunarYear = GetcYearString();
    var lunarMonth = '';
    if (cMonth < 1) {
        lunarMonth += "(闰)";
        lunarMonth += monString.charAt(-cMonth - 1);
    } else {
        lunarMonth += monString.charAt(cMonth - 1);
    }
    lunarMonth += "月";
    ...
    var lunarDay = '';
    lunarDay += (cDay < 11) ? "初" : ((cDay < 20) ? "十" : ((cDay < 30) ? "廿" : "三十"));
    if (cDay % 10 != 0 || cDay == 10) {
        lunarDay += numString.charAt((cDay - 1) % 10);
    }

    //tmp += " " + SolarTerm(new Date(solarYear, solarMonth, solarDay));

    var obj = {};
    obj.cYear = GetcYearString();
    obj.cMonth = lunarMonth;
    obj.cDay = lunarDay;
    obj.solarTerm = SolarTerm(new Date(solarYear, solarMonth, solarDay));
...
//获取年月日及节气，这里所需的月份值都是从0开始+1的，相当于现实中的月份的值
function GetLunarDay(solarYear, solarMonth, solarDay) {
    if (solarYear < 1921 || solarYear > 2020) {
        ...
        return GetcDateString(solarYear, solarMonth, solarDay);
    }
}

//只获取年份，这里所需的月份值都是从0开始+1的，相当于现实中的月份的值
